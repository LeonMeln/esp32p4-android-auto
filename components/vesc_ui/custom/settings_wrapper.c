...
/* lv_conf.h was included to pick up LV_REALDEVICE. On this embedded
 * build LVGL is configured via Kconfig (CONFIG_LV_CONF_SKIP=y) and there
 * is no lv_conf.h on the include path — LV_REALDEVICE stays undefined,
 * so SIMULATOR_MODE = 1 and settings live in RAM. */
