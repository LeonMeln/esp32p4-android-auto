...
/* Original head-unit credentials from aasdk (Cryptor.cpp).
 *
 * This is the well-known JVC Kenwood × Google Automotive Link cert that
 * every open-source AA head unit (aasdk, openauto, headunit-revived)
 * presents to gearhead. Its signature was issued by the real Google AA
 * CA which gearhead trusts; replacing it with any self-signed cert
 * triggers gearhead's "Communication error 7".
 *
 * The certificate's UTCTime fields use the legacy "YYMMDDHHMMSS±HHMM"
 * format, which RFC 5280 forbids and stock mbedTLS rejects. We bypass
 * that by linker-wrapping mbedtls_x509_get_time() in aa_x509_patch.c.
 *
 * Source: research/_sources/aasdk/src/Messenger/Cryptor.cpp
 */

static const char AA_CERTIFICATE_PEM[] =
    "-----BEGIN CERTIFICATE-----\n"
...
static const char AA_PRIVATE_KEY_PEM[] =
