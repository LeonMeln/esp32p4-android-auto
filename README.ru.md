...
| **D1 Mini ESP32-WROOM-32** | бонус AA | Любая USB-C плата на ESP32 с Classic BT. У ESP32-P4 / C6 нет BT Classic. ~$3. |
| Провода-перемычки, USB-C кабели | всегда | |
| **CR2032 батарейка** в посадочное место H8 | опционально | Бэкап RTC (часы переживают отключение питания) |

Если Android Auto не нужен — D1 Mini можно вообще не ставить, дашборд
...
Нужна только если хочется Android Auto бонус. Делается **один раз**, дальше
P4 сам перепрошивает агент по UART, как только встроенная версия становится
новее.
...
### Android Auto бонус (после прошивки D1 Mini)
...
| VESC LISP poll | ✅ | Индикатор круиза + кастомные счётчики |
