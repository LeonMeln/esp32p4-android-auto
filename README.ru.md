...
- **Прошивка по WiFi или BLE** — образ P4 зашит в APK и заливается в head unit
  по его SoftAP (креды читаются по BLE автоматически, либо вводятся / сканируются
  вручную) или прямо по BLE-каналу.
- **Файловый менеджер устройства** — просмотр хранилища head unit'а (`/vescfs`
  внутренний LittleFS + microSD) с телефона по BLE: список, скачивание, загрузка,
  переименование, удаление. Именно так на устройство кладётся стартовый GIF.

### 🗂️ Файловый браузер на устройстве + стартовый splash
- **Файловый браузер** на самом head unit'е (*Settings → Files*) — тач-интерфейс,
  ходит по `/vescfs` (внутренний) и `/sdcard` (microSD): папки, размеры, превью
  картинок, переименование / перемещение / удаление.
- **Стартовый splash** — положи анимированный `splash.gif` в `/vescfs`, и он
  играет на весь экран при загрузке, затем уступает место дашборду. Нет файла —
  обычная загрузка.
...
### Файлы устройства и стартовый GIF

Два способа управлять файлами на head unit'е:

- **На устройстве**: *Settings → Files* — тач-браузер по `/vescfs` (внутреннее
  хранилище) и `/sdcard` (microSD, если вставлена). Просмотр, превью картинок,
  переименование / перемещение / удаление.
- **С телефона**: *Home → Device files* (показывается при подключении к прошивке
  с поддержкой) — список папок, скачивание на телефон, загрузка с телефона,
  переименование, удаление, mkdir.

**Поставить стартовый GIF**: залей анимированный GIF в `/vescfs` через *Device
files* и назови его **`splash.gif`** (загрузи сразу с этим именем или переименуй
после загрузки). На следующей загрузке он проиграется на весь экран, затем
включится дашборд. Держи его небольшим — `/vescfs` это ~4 МБ на Waveshare и
~1 МБ на jc4880.
...
| Приложение — прошивка по WiFi / BLE | ✅ | Зашитый образ POST'ится на OTA-эндпоинт SoftAP, либо стримится по BLE-каналу |
| Файловый браузер на устройстве (`/vescfs` + microSD) | ✅ | Тач-UI в *Settings → Files*; превью картинок, переименование / перемещение / удаление |
| Приложение — файловый менеджер по BLE | ✅ | Просмотр / скачивание / загрузка / переименование / удаление файлов устройства с телефона |
| Стартовый splash GIF | ✅ | Анимированный `/vescfs/splash.gif` на загрузке (заливается через файловый менеджер приложения) |
